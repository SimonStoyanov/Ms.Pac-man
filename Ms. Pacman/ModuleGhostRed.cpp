...
update_status ModuleGhostRed::Update()
{
	...
	{
		if ((int)(position.x + 7) == (p_mid.x * 8) + 4 && (position.y - 7) >= (p_mid.y * 8) + 2 && (position.y - 7) <= (p_mid.y * 8) + 6)
		{
			can_go_right = true;
		}
	...
	{
		if ((int)(position.x + 7) >= (p_mid.x * 8) + 2 && (position.x + 7) <= (p_mid.x * 8) + 6 && (position.y - 7) == (p_mid.y * 8) + 4)
		{
			can_go_up = true;
		}
