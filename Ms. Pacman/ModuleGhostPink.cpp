...
ModuleGhostPink::ModuleGhostPink()
{
	...
	right.PushBack({ 17, 97, 14, 14 });
	right.speed = 0.10f;

	// left animation
	left.PushBack({ 113, 97, 14, 14 });
	left.PushBack({ 97, 97, 14, 14 });
	left.speed = 0.10f;

	// up animation
	up.PushBack({ 65, 97, 14, 14 });
	up.PushBack({ 81, 97, 14, 14 });
	up.speed = 0.10f;

	// down animation
	down.PushBack({ 33, 97, 14, 14 });
	down.PushBack({ 49, 97, 14, 14 });
	down.speed = 0.10f;
...
update_status  ModuleGhostPink::Update()
{
	...
	if (!App->player->is_dead)
	{
		...
		{
			if (now - passed_box > 6 * 0.5f * 1000.0f) // positioning ////////
			{
				if (position.y > 99)
				{
					position.y -= 0.5f;
